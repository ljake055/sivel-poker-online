...
function awardUncontested(room, winner) {
  ...
  game.result = { title: `${winner.name} wins`, detail: `${amount} chips · uncontested`, winners: [winner.seat] };
...
function finishTable(room) {
  ...
  game.result = { title: `${winner.name} wins the table`, detail: `${winner.chips} chips`, winners: [winner.seat], tableOver: true };
  log(room, game.status);
  broadcast(room);
}
